package Mmsite::Lib::Members::Auth::Vkcom;
######################################################################################
...
sub vk_get_user_name {
    ...
    to_log( $PATH_LOG_VK, "VK", "Request: " . "https://api.vk.com/method/users.get?owner_id=$vkid&v=$AUTH_VK_VER&access_token=$vksess&fields=first_name,last_name" );
  
    # если запрос не удался, то выходим
    if ($code != 200) {
        to_log( $PATH_LOG_VK, "VK", 'error: получен код ' . $code );
    ...
    to_log( $PATH_LOG_VK, "VK", 'get: ' . $data );

    # разбираем данные
    unless (eval { $json_data = $json_xs->decode($data) } ) {
        to_log( $PATH_LOG_VK, "VK", 'error: не удалось распарсить json' );
...
sub _vk_auth_error {
    ...
    $error = $$ref{'error_msg'};
    on_utf8(\$error);
    to_log( $PATH_LOG_VK, "VK", 'error: ошибка авторизации: code=' . $$ref{'error_code'} . ', msg=' . $$ref{'error_msg'} );
    return $error;
}
